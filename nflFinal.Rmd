...
write.csv(df_WR, file = "NFL_Combine_Performace.csv", row.names = FALSE)
...
WR_1Q <- WR_Allometric %>% filter(hwRatio == "1st Quartile")
WR.Heavy <- WR_Allometric %>% filter(WeightQ == "Heavy Half")

# Plots: Comparing Key Groups (Lengthy, Heavy, 1st Quartile)
g1 <- ggplot(WR.Tall, aes(x = X3cone, y = YPG)) + 
...
g2 <- ggplot(WR.Heavy, aes(x = X3cone, y = YPG)) + 
...
g3 <- ggplot(WR_1Q, aes(x = X3cone, y = YPG)) + 
  geom_point(col = "red2") + 
  stat_smooth(method = "lm", se = FALSE, formula = y ~ x, col = "red2") +
  ggtitle("1st Quartile Height:Weight Ratio") + 
  xlab("3-Cone Drill (Seconds)") + 
  ylab("YPG")
...
model1 <- lm(YPG ~ X3cone, data = WR_Allometric)

# Add Residuals to the Dataset
WR_Allometric$Residuals <- residuals(model1)

# Residual Plot: YPG vs. 3-Cone Drill
ggplot(WR_Allometric, aes(x = X3cone, y = Residuals)) + 
  geom_point() + 
  stat_smooth(method = "loess", se = FALSE, span = 1.5) +
  ggtitle("Residual Plot: YPG vs. 3-Cone Drill") + 
  xlab("3-Cone Drill (Seconds)") + 
  ylab("Residuals")
